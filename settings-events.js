...
const events=[
 {name:'APEX SPRINT',desc:'Score 7,500 points in one run',target:7500,type:'SCORE',reward:500},
 {name:'GEM RAID',desc:'Collect 35 gems',target:35,type:'GEMS',reward:450},
 {name:'PERFECT HUNTER',desc:'Reach x8 combo',target:8,type:'COMBO',reward:600},
 {name:'GUARDIAN BREAKER',desc:'Defeat a Guardian',target:1,type:'BOSS',reward:750},
 {name:'LONG HAUL',desc:'Survive 90 seconds',target:90,type:'TIME',reward:800},
 {name:'ELITE PURGE',desc:'Defeat 12 elite enemies',target:12,type:'ELITE',reward:650}
];
...
function build(){
 ...
 if(nav&&!document.querySelector('[data-panel="daily"]')){const mk=(id,text)=>{const b=document.createElement('button');b.className='tab';b.dataset.panel=id;b.type='button';b.textContent=text;return b};nav.append(mk('daily','DAILY EVENT'),mk('settings','SETTINGS'))}
 ...
 if(nav&&!nav.dataset.settingsBound){nav.dataset.settingsBound='1';nav.addEventListener('click',e=>{const b=e.target.closest('.tab');if(!b)return;document.querySelectorAll('.tab').forEach(x=>x.classList.toggle('active',x===b));document.querySelectorAll('.panel').forEach(x=>x.classList.toggle('active',x.id===b.dataset.panel))})}
}
function renderDaily(){const d=daily(),card=$('dailyCard');if(!card)return;const pct=Math.min(100,d.progress/d.target*100);card.innerHTML=`<article class="dailyCard"><div class="dailyTop"><span>LIVE · ${dayKey()}</span><strong>+${d.reward} GEMS</strong></div><h3>${d.name}</h3><p>${d.desc}</p><div class="dailyProgress"><i style="width:${pct}%"></i></div><div class="dailyBottom"><b>${Math.min(d.progress,d.target).toLocaleString()} / ${d.target.toLocaleString()}</b><button id="claimDaily" ${d.claimed||d.progress<d.target?'disabled':''}>${d.claimed?'CLAIMED':'CLAIM REWARD'}</button></div></article>`;
 $('claimDaily')?.addEventListener('click',()=>{const n=daily();if(n.claimed||n.progress<n.target)return;try{const raw=JSON.parse(localStorage.getItem('DINO_LEGENDS_V30')||'{}');raw.gems=(raw.gems||0)+n.reward;localStorage.setItem('DINO_LEGENDS_V30',JSON.stringify(raw));n.claimed=true;writeDaily(n);renderDaily();if($('gems'))$('gems').textContent=(raw.gems||0).toLocaleString()}catch{}});
...
function renderSettings(){const g=$('settingsGrid');if(!g)return;g.innerHTML='';const items=[['volume','MASTER VOLUME',`${meta.volume}%`],['particles','PARTICLES',meta.particles?'ON':'OFF'],['shake','SCREEN SHAKE',meta.shake?'ON':'OFF'],['reducedMotion','REDUCED MOTION',meta.reducedMotion?'ON':'OFF'],['autoPause','AUTO PAUSE',meta.autoPause?'ON':'OFF']];items.forEach(([key,label,val])=>{const row=document.createElement('div');row.className='settingRow';row.innerHTML=`<div><b>${label}</b><small>${key==='volume'?'Audio level':'Accessibility / gameplay preference'}</small></div><button data-setting="${key}">${val}</button>`;g.append(row)});g.querySelectorAll('button').forEach(b=>b.onclick=()=>{const k=b.dataset.setting;if(k==='volume')meta.volume=meta.volume>=100?0:meta.volume+10;else meta[k]=!meta[k];save();renderSettings();document.dispatchEvent(new CustomEvent('dino-settings-changed',{detail:{...meta}}))});const reset=document.createElement('div');reset.className='settingRow danger';reset.innerHTML='<div><b>RESET GAME SAVE</b><small>Deletes progression and starts fresh.</small></div><button id="resetSave">RESET</button>';g.append(reset);$('resetSave').onclick=()=>{if(confirm('Reset all Dino Legends progression?')){localStorage.removeItem('DINO_LEGENDS_V30');location.reload()}}}
function updateDaily(){const d=daily();let p=d.progress;if(d.type==='SCORE')p=Number($('score')?.textContent.replace(/,/g,'')||0);else if(d.type==='COMBO')p=parseFloat(($('combo')?.textContent||'x1').replace('x',''))||0;else if(d.type==='GEMS')p=Math.max(p,Number($('gems')?.textContent.replace(/,/g,'')||0));else if(d.type==='TIME')p=Math.max(p,Number(document.body.dataset.runTime||0));if(p>d.progress){d.progress=p;writeDaily(d);renderDaily()}}
